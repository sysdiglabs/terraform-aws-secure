...
| <a name="input_org_units"></a> [org\_units](#input\_org\_units) | TO BE DEPRECATED: Please migrate to using `include_ouids` instead.<br>When set, list of Organization Unit IDs in which to setup EventBridge. By default, EventBridge will be setup in all accounts within the Organization.<br>This field is ignored if `is_organizational = false` | `set(string)` | `[]` | no |

...
# First create a new stack set for API Connection and Destination for the organization
resource "aws_cloudformation_stack_set" "api_connection_destination_org_stackset" {
  count = var.is_organizational ? 1 : 0

  name             = join("-", [local.eb_resource_name, "ApiDestinationOrg"])
  tags             = var.tags
  ...
  template_body = templatefile("${path.module}/api_destination_template_body.tpl", {
...
resource "aws_cloudformation_stack_set" "eb-rule-stackset" {
  ...
  depends_on = [
    aws_cloudformation_stack_set.api_connection_destination_org_stackset,
  ]
}
...
resource "aws_cloudformation_stack_set_instance" "api_connection_destination_org_instance" {
  for_each = var.is_organizational ? local.region_set : toset([])
  region   = each.key

  stack_set_name = aws_cloudformation_stack_set.api_connection_destination_org_stackset[0].name
  deployment_targets {
...
resource "aws_cloudformation_stack_set_instance" "eb_rule_stackset_instance" {
  ...
  depends_on = [
    aws_cloudformation_stack_set_instance.api_connection_destination_org_instance
  ]
}
